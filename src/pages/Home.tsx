...
export function Home() {
  ...
  const createTarefa = async () => {
    setDone(false);
    await addDoc(tarefasCollectionRef,  { name: newName, done: newDone  }).then(() => getTarefas()).catch((error) => console.log(error))
    setNewName('')
  }
  ...
  const deleteTarefa = async(id: any) => {
    ...
    setValueDelete(id)
    await deleteDoc(tarefaDoc).then(() => getTarefas()).catch((error) => console.log(error) );
  }
  ...
  const updateTarefa = async (id: any, name: string) => {
    ...
    setValueUpdate(id)
    await updateDoc(tarefaDoc, newFields).then(() => getTarefas()).catch((error) => console.log(error))
  }
    ...
    <C.Container>
      <C.Area>
        ...
        <div className="image">✚</div>
          ...
          {list.map((item, index) =>(
            <ListItem key={index} item={item} deleteTarefa={deleteTarefa} updateTarefa={updateTarefa}/>
          ))}
      </C.Area>
    </C.Container>
  );
